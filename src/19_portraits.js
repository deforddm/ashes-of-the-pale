...
function drawPortrait(ctx, id, W, H, t){
  ...
  const skin = {sgt:'#a8826a', brisk:'#b08c76', kettle:'#c29a7e', tuft:'#c9b5a6', ohl:'#a8896f', ellis:'#b89a80'}[id] || '#a8896f';
  ...
  // shoulders / torso
  const wide = id === 'brisk' ? 52 : id === 'sgt' ? 44 : id === 'ohl' ? 42 : id === 'ellis' ? 34 : 36;
  const torsoCol = {sgt:['#6b4a2e','#2e2014'], brisk:['#8a8880','#2c2b29'], kettle:['#8a4a2a','#33190f'], tuft:['#4a4358','#161320'], ohl:['#5a6b58','#1c231b'], ellis:['#5a6b4a','#1f2618']}[id] || ['#4a4a44','#1a1a18'];
  ...
  if (id === 'ellis') { ctx.strokeStyle = '#8b7355'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(-30, -48); ctx.quadraticCurveTo(-44, -20, -30, 4); ctx.stroke(); ctx.strokeStyle = '#cfc8b8'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(-30, -48); ctx.lineTo(-30, 4); ctx.stroke(); ctx.fillStyle = '#2a2018'; ctx.fillRect(18, -22, 14, 22); ctx.strokeStyle = 'rgba(0,0,0,.4)'; ctx.lineWidth = 1; for (let i=0;i<3;i++){ ctx.beginPath(); ctx.moveTo(20 + i*4, -22); ctx.lineTo(20 + i*4, 0); ctx.stroke(); } }
  ...
  const eyeCol = id === 'tuft' ? '#c9bbff' : id === 'ohl' ? '#cfe8d8' : id === 'ellis' ? '#d8e8d0' : '#e6dac2';
  ...
  if (id === 'ellis') { ctx.beginPath(); ctx.arc(0, hy-3, hr+1, Math.PI*1.02, Math.PI*1.98); ctx.fillStyle = lit('#2a2018','#0c0906',-hr,hr); ctx.fill(); poly(ctx, [[-hr-1,hy-4],[-hr+2,hy+6],[-hr+5,hy-2]], '#1a1410'); poly(ctx, [[-8,hy+14],[8,hy+14],[14,hy+22],[-14,hy+22]], lit('#4a5a3c','#1c2216',-14,14)); }
