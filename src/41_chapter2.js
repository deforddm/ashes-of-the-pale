...
`Day three. Somewhere ahead, a week ahead, a city with blue fire in its streets. Somewhere behind, a camp that has stopped burning. Somewhere on the same road, though nobody has said so, a captain who should have died twice.`],
  ...
  areas:[
    ...
    { id:'hound_site', title:'The Rhivi Plain · where the horses died', sub:'Day three · dusk', hint:'Tap ground to move · tap a figure to talk · east is the way', decor:'plain_dusk',
      map:[ "################",
            "#,,.....,.....,#",
            "#..r.......,...#",
            "#.....,........#",
    ...
    { id:'ridge', title:'The Rhivi Plain · the fourth camp', sub:'Night · a low ridge with the wind on it', hint:'Tap ground to move · tap a figure to talk · the fire is lit', decor:'plain_night',
      ...
      quest:()=> S.f.c2_lightDone ? 'Dawn is coming. East, and the hills.' : S.f.c2_light ? 'There is a light in the west.' : !S.f.c2_sethNight ? 'The fourth camp. Sethand is at the fire, which is new.' : 'Night. Nobody is sleeping yet.',
  ...
  gear:{ // slot ∈ weapon|armour|trinket. who = ids that can wear it, or null for anyone.
    ...
    toccloak:{name:'Toc\'s spare cloak', slot:'armour', who:['ellis','tuft','sgt'], ac:1, line:'A Claw\'s field cloak, grey going brown, with the shoulder cut for a bow. Toc left it on the saddle and did not look back at it.'} },
...
Sethand is out ahead on his own horse, a small tough thing the colour of the grass, and has been since dawn. He does not look back to see if you're following. He has said that the road is the road, and that a Malazan could not lose it if he were paid to, and that he has known Malazans who were paid to.

Kettle walks beside the wagon with a hand on the oilcloth, as if the munitions might climb out. Tuft walks where the grass is tallest. Ohl rides, because his knees have made the decision for him, and reads nothing.
...
${S.f.c1_plant || S.f.knowTruth ? `Behind you, Tuft says nothing, very clearly.` : `Kettle, hopeful: "Opinions."`}`,
...
Sethand has moved his horse to the far side of the wagon, and has not said anything, and is not going to.`,
      ch:[{t:'Leave it.'}]} : {sp:'The barrow', fx:()=>{S.f.c2_barrowSeen=1;}, txt:
...
Kettle has already found the gap. Of course she has.`,
      ch:[{t:'Look at the fallen stone.', check:['wits',12], go:'c2_barrow_open', fail:'c2_barrow_miss'},
          {t:'Go in.', go:'c2_barrow_in'},
          {t:'"Leave it. Sethand said."', fx:()=>{S.f.c2_sethTrust = (S.f.c2_sethTrust || 0) + 1; loy('ohl',1); loy('kettle',-1);}, go:()=>startExplore()}]},
...
Tuft has moved to where she can see the gap and you can't see her. She does that. "Something came out," she says. "Then something went back. The going-back is newer."`,
      ch:[{t:'Go in anyway.', go:'c2_barrow_in'},
          {t:'"Nothing that opens its own grave is worth the walk. Leave it."', fx:()=>{S.f.c2_sethTrust = (S.f.c2_sethTrust || 0) + 1; loy('brisk',1); loy('kettle',-1);}, go:()=>startExplore()}]}),
...
Kettle is halfway into the gap already, feet first, which is how she goes into anything. Her voice comes back hollow: "There's a room, Sergeant. There's a *room.*"`,
      ch:[{t:'Go in after her.', go:'c2_barrow_in'},
          {t:'"Kettle. Out. Now."', fx:()=>{loy('kettle',-1); loy('ohl',1);}, go:()=>startExplore()}]}),
    c2_barrow_in:()=>({sp:'The barrow', scene:'dark', txt:
...
Tuft, very quietly: "They're not undead. They're *worse* than undead. They're patient."`,
      ch:[{t:'"Shields. Back to the passage."', go:()=>startBattle('barrow',{surprise:'e'})},
          {t:'Kettle has a burner in her hand.', tag:'uses 1 burner', req:()=>S.inv.burner>0, fx:()=>{S.inv.burner--;}, go:()=>startBattle('barrow',{pre:true})}]}),
    c2_after_barrow:()=>({sp:'The barrow', scene:'dark', fx:()=>{ S.f.c2_barrowFought=1; gain('barrowtorc'); }, txt:
...
"*Was.*"`,
    ...
    c2_toc_hound:()=>({sp:'Toc the Younger', fx:()=>{S.f.c2_tocHound=1;}, txt:
...
He stands. It costs him something; the burn pulls.
...
${S.f.c2_prints ? `"You saw the prints. Where they stop." Not a question. "Don't tell her. She's been not-looking at them for an hour."` : `"There's a place over there where the ground's wrong. Don't let your people stand on it. I don't know why. I just wouldn't."`}
...
"Sergeant." She's heard Toc; she's got the rank without looking. "It's a mare. Nine years. She was the only thing on this plain that could outrun that, and she tried, and she was wrong."

Her voice is very exact. Every word is put down where it's meant to go, and none of them shake.

"He's asked you. I know. I'd like it noted that I didn't."
...
Ohl has come up on the horse's other side, slow, the way he does with everything that's dying, and put a hand flat on the mare's neck, and is saying something in Ehrlii that isn't for you.`,
      ch:[{t:'Do it yourself. Quick, and once.', check:['might',11], go:'c2_horse_might', fail:'c2_horse_fail'},
...
Toc stands. He does it himself, with his knife, one-handed, badly, because the burn pulls, and the mare takes longer than she should have. Ellis holds the head the whole time. When it's done she lays it down and stands and takes her glove off and looks at the hand under it, burned, fused at two knuckles, and puts the glove back on, and looks at you.
...
Toc has his own horse's saddle over his shoulder and is standing by Sethand's, negotiating, in the way Toc seems to negotiate, which is to say he asks and then waits until the other man is embarrassed. Sethand hands over the reins of the pack-pony without a word. He'll want something for that. Later.
...
${SQUAD().includes('ohl') && S.f.c2_horseHow === 'tea' ? `Ohl has the oilcloth out. He isn't writing on it. He's looking at the place where he would.` : `Tuft has gone to the edge of the flattened grass and is standing with her back to it, which is the way she stands to things she is listening to.`}`,
      ch:[{t:'Toc wants a word.', go:'c2_toc_offer'}]}),
...
${S.f.clawFavour ? `"You held a cadre tent for a grey cloak at the Pale." Not accusing. Exact. "I know the one. He wrote my transfer. He writes very neatly." A pause. "I'd want to know, Sergeant, before I said yes, whether you'd hold a tent for him again. I'd rather you told me the truth and I didn't like it."` :
...
${S.f.clawFavour ? `Then, quieter: "I'll be watching you. I'd want you to know that. It isn't personal. It's training."` : ``}`,
    ...
    c2_ellis_yes:()=>({sp:'Where the horses died', fx:()=>{ S.f.c2_ellisJoined=1; recruit('ellis'); note('Ellis joins the Fourth.','good'); }, txt:
...
Kettle: "You've done that count already."
...
Ohl has gone back to the wagon. He's not on the driver's board. He's standing behind it, where nobody can see his face, and Brisk has moved so that nobody can.`,
      ch:[{t:'"Ride safe, Toc."', go:'c2_toc_rides'}]}),
    c2_toc_rides:()=>({sp:'Toc the Younger', fx:()=>{ S.f.c2_tocGone=1; gain('toccloak'); }, txt:
`He turns the pony north-west, toward the place on the horizon where Sethand said the dust was. He doesn't hurry. A scout doesn't.
...
There's a cloak on the wagon's tailboard that wasn't there before. Grey going brown, the shoulder cut for a bow. ${S.f.c2_ellisJoined ? `Ellis looks at it and doesn't touch it. "His spare," she says. "He never wears it. He says it's for when he's cold, and he's never cold. That's a Claw joke. There aren't many."` : `Nobody claims it. Brisk folds it and puts it under the oilcloth with the rest of what the Bridgeburners left.`}

Sethand, from his horse, to nobody: "The ridge before dark. I said."`,
...
`Sethand is waiting by the ridge. Toc is still sitting with the horse. Whatever this is, it isn't finished, and you're the one who'll have to finish it.`,
      ch:[{t:'Not yet.'}]},
...
"Something is going to happen tonight. I do not know what. The horses know. The clans have moved. The stars are wrong, and you have noticed that, and not said it, which I respect."`,
      ch:[{t:'"The dust-line. It\'s closer."', go:'c2_seth_night_dust'},
          ...
          {t:'Say nothing. Let him have the fire.', fx:()=>{S.f.c2_sethTrust = (S.f.c2_sethTrust || 0) + 1;}, go:'c2_seth_night_quiet'}]}),
...
"And because a man who can put a horse down that gently should not sit alone." A nod toward the wagon, toward Ohl. "That is all. I will go back to my own fire when I have finished being here."`,
...
${!(S.f.c2_drawn || S.f.c2_noCard) ? `He nods, very slightly, at Tuft, and the Deck in her hand.` : `He is looking west. He has been looking west for some time.`}`,
    ...
    c2_fire:()=> S.f.c2_drawn || S.f.c2_noCard ? {sp:'The fire', txt:
...
${SQUAD().includes('ellis') ? `Ellis, from the dark beyond the stakes, not loudly: "That thing got a man killed at Pale. A cadre mage read it three nights running and on the fourth the Hounds came." A pause. "I'm not saying it's the Deck's fault. I'm saying I've never seen it help." Tuft doesn't answer her. Tuft doesn't look at her. It is, you realise, the first thing Ellis has said that Tuft has heard.` : `Kettle: "Draw it. If it's bad we'll know it's bad. If it's good, Brisk can tell us why it's bad anyway."`}`,
      ch:[{t:'Let her draw.', fx:()=>{ S.f.c2_drawn=1; S.card = ['raven','raven','raven','oponn','obelisk','knight'][R(6)]; }, go:()=>cardSequence(()=>talk('c2_card'))},
          ...
          {t:'Not yet.'}]},
    c2_card:()=>{ const c = (CARDS[S.card] || CH2.card); return {sp:'The Deck of Dragons', scene:'fire', txt:
...
${S.f.c1_noCard ? `"That's twice." Not an accusation. An observation, filed.` : `Kettle, to the fire: "It'd have been the Raven. It's always the Raven out here. You can hear them at night."`}`,
...
`${S.f.c2_tentSeen ? `The tents are where they were. Kettle's snoring is not louder than the wind, but it's arguing with it.` :
...
`${S.f.c2_wagonNight ? `Brisk is asleep against the wheel, or doing a very good impression. Her spear is across her knees. Pell the mule is asleep too, and looks, for once, like it doesn't hate anyone.` :
...
Not Sethand. Not Ellis out beyond the stakes with the best eyes on the plain. Tuft, at the fire, with her back to the west, stands up before anything has happened and turns round and says "No," once, quite quietly, the way you'd say it to someone across a room.
...
"That's her." Tuft's voice is nothing. "Sergeant, that's Tattersail. That's Telas, that's her warren, she's *burning* it, she's burning all of it at once, there's nothing left after that, there's nothing you can do after that—"

Sethand, hoarse: "A mage. Dying. Two mages. One of them is very large."

Brisk hasn't moved. Brisk is looking at you.`,
      ch:[{t:'"Saddle the mule. We ride to it."', go:'c2_ride'},
...
"Sergeant." That's all. The whole of the argument. Then she picks up her shield and the spear, and goes to the wagon, and starts hitching Pell, because if the Fourth is going to be stupid it is going to be stupid with its rations.
...
You ride. The light doesn't get closer for a long time and then it does, all at once, and the grass ahead is lit like day and there are riders in it.`,
...
The one in front says something in Rhivi. It has your sergeant's tone in it. Sethand answers, short. The man says it again, and this time he points at Kettle's satchel, and then at the light, and then at you, and the meaning is very clear in any language: *not one step*.

${SQUAD().includes('ellis') ? `Ellis, at your stirrup, without moving her lips: "Eleven. Four more in the grass on the left. They're not here for us. They're here for whatever's behind them."` : `Tuft, at your stirrup: "Sergeant. They're afraid of us. Not of the fire. Of *us.* Malazans and a burning mage. Think about how that looks."`}`,
...
"Look," he says, not turning. "Do not touch. And do not ask what they carry. And do not, Malazan, ever tell anyone I brought you here."`,
      ch:[{t:'Ride through.', go:'c2_ashes'}]}),
    c2_ashes:()=>({sp:'The ashes', scene:'plain_night', fx:()=>{S.f.c2_ashes=1;}, txt:
...
Ohl has the oilcloth open on his saddle. He writes. One line. He closes it. "She was not mine to write, Sergeant. I wrote her." His hand is steady. His voice isn't.
...
"She is the first name on that list I could not have saved. Every other one, I was there. I had my hands in them. Her I was two hours away and asleep." He folds the oilcloth along its old creases. "I have decided that counts. I have decided the list is not a list of my failures. It is a list of the ones I would have tried for. That changes it. I am fifty-eight and I have changed it tonight, on a horse, and I would like you to remember that I did, because I will not."
...
${SQUAD().includes('ellis') ? `Ellis comes in from the stakes an hour before dawn. "Sergeant. Riders. Three. On foot, walking, from the west, and they're not Rhivi, and I couldn't see them until they wanted me to."` : `Sethand, an hour before dawn, on his feet with his knife out: "Malazan. Get up. Something is coming across the grass, and it is not walking like anything I know."`}`,
    ...
    c2_andii:()=>({sp:'Tiste Andii', scene:'plain_night', fx:()=>{S.f.c2_andii=1;}, txt:
...
"Crone, little soldiers. Eldest of the Great Ravens, and the one Anomander Rake sends when he wants a thing *seen*, because ravens see, and we remember, and we are so very hard to kill." It preens, once, an old woman settling a shawl. "I have been over the whole of this plain tonight and you are the most interesting thing on it, and that is not a compliment. Interesting things get *eaten*."`,
      ch:[{t:'"What did you see? Out there."', go:'c2_crone_saw'},
          {t:'"Get off my wagon."', fx:()=>loy('brisk',1), go:'c2_crone_off'}]}),
...
"And a woman with a thing walking beside her that is not a man, going to the hills, going to *wake* something. And a captain, gone into the ground with a Hound, and not dead, which is annoying; I had the eyes marked. And nine soldiers who went south by quorl and have gone under a city like rats under a floor." The head cocks. "Your friends. You'll be a week behind them. You'll be *late*, Malazan. Everyone is going to be late to what is coming, and you will be the latest, and it may be that that is the only reason you'll live through it."`,
      ch:[{t:'"Get off my wagon."', go:'c2_crone_off'}]}),
...
${S.f.c2_key === 'light' ? `"Sergeant." The voice comes down clear. "The girl on her knees on the glass. Keep her. Not for her sake. For what she'll be worth to someone, and it will not be you, and it will not be me, and I would *so* like to see the face of whoever it is."` : `"Sergeant." The voice comes down clear. "You counted your bread while she burned. Good. Keep counting. Somebody in this war is going to need a soldier who does what he's told, and my lord has none, and it is his only weakness, and I have told him so."`}
...
`${S.f.c2_key === 'light' ? `It's full day by the time you're back at the ridge, and the ridge is a ridge, and the tents are where you left them, and Brisk strikes them without a word and without looking at anyone and loads the wagon and hitches Pell and stands by the tailboard waiting, and the waiting is a sentence with your name in it.

"A day," she says, when you're close enough. "That's what that cost. Whiskeyjack's *week* is a day shorter, and he'll know, because he's the kind that knows, and he'll ask, and I'll tell him."
...
Brisk has the tents struck and the wagon loaded and Pell hitched before the sun's clear of the horizon. She is very efficient this morning. She is efficient the way a woman is who has been proved right and would rather not have been.
...
`Two days of grass, and then it changes: shorter, greyer, with stones coming up through it like knuckles. Sethand says this is where the road ends and the hills begin, and that the hills are Gadrobi, and that the Gadrobi are not Rhivi, and that this is the last thing he will explain to a Malazan.`,
      ch:[{t:'Into the hills.', go:()=>{ startExplore('hills_edge'); talk('c2_hills'); }},
          {t:'Not yet.'}]} : S.f.c2_light ? {sp:'East · the hills', txt:
...
${S.f.c2_key === 'light' ? `Brisk, at the wagon: "A week, he said. It'll be a week and a day. You can tell him why or I can, Sergeant, and you'll do it better, and I'll do it first."` : `Brisk, at the wagon: "A week, he said. It's a week. Somebody write that down."`}`,
      ch:[{t:'"Sethand. The dust-line. You said you\'d tell me at the hills."', go:'c2_hills_dust'},
...
${(S.f.c2_sethTrust || 0) >= 2 ? `And then, without looking back: "You listened. Three times, when I said *do not*. That is three more than any Malazan. If the Fourth comes back across the plain, come by the Mhybe's fires. Say my name. It will be worth something, once."` : `He doesn't say anything else. He has said, by his count, more than enough.`}`,
...
"${SQUAD().includes('ellis') ? 'Nine days, six.' : 'Twelve days, five.'} Mule's Pell. ${S.f.c2_badge ? 'One badge, Ninth Regiment, Second Army, unclaimed.' : ''}${S.f.c2_key === 'light' ? ' One day lost, west, on the sergeant\'s order.' : ' No days lost.'}"

She closes it. "That's the chapter, Sergeant. Whiskeyjack can read it or not."

${S.f.c2_key === 'light' && S.loy.brisk <= -1 ? `She doesn't look at you when she says it. She hasn't, since the ridge. It's not anger. Brisk doesn't do anger. It's a ledger, and you're in it, and the line under you hasn't been drawn yet.` : `Kettle, from the mule's head: "Put the cusser in. Gerrun. Or Maud. Whichever's still with us." Brisk doesn't. Kettle knew she wouldn't.`}`,
      ch:[{t:'Leave'}]}),
...
${SQUAD().includes('ellis') ? `Ellis is out ahead, on foot, where the ruts go up into the hills, with her bow strung and her burned hand gloved. She looks back once. Counts. Arrives at six, and checks it.` : S.f.c2_ellisRefused ? `Somewhere north-west, a scout with one eye is riding toward an Adjunct with a woman who could have been the Fourth's, and Ohl has not mentioned it, and will not, and the not-mentioning has a shape.` : ``}
