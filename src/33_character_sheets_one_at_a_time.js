...
function openChars(i, tab){
  ...
  const soldier = `<div class="sec"><h4>Measure</h4><div class="grid2">${bar('Might', c.st.might)}${bar('Wits', c.st.wits, 'w')}${bar('Guile', c.st.guile, 'g')}<div class="stbar"><span>Level</span><div class="bar"><i style="width:${S.lvl/8*100}%"></i></div><span>${S.lvl}</span></div></div></div>
    ...
    <div class="sec"><h4>Field rules</h4><p class="fine">Flanking: +2 to hit when an ally stands next to the target on the far side, melee or ranged. Stepping out of an enemy's reach gives that enemy one free swing a turn; moving from one threatened tile to another does not. Once you have moved and then acted, your movement for the turn is spent. The deserters know both tricks.</p></div>
