...
function drawExplore(t){
  ...
  if (a.decor && (a.decor.startsWith('plain') || a.decor.startsWith('hills'))) {
    ...
    a.map.forEach((row, y) => [...row].forEach((ch, x) => { const cx = x*T + T/2, cy = y*T + T/2;
  ...
  const v = ctx.createRadialGradient(cols*T/2, rows*T/2, T*4, cols*T/2, rows*T/2, T*10); v.addColorStop(0, 'rgba(0,0,0,0)'); v.addColorStop(1, `rgba(0,0,0,${a.decor === 'pale' ? .6 : a.decor === 'plain' || a.decor === 'hills' ? .35 : a.decor === 'plain_dusk' || a.decor === 'hills_dusk' || a.decor === 'city_dusk' ? .5 : a.decor === 'city_night' || a.decor === 'roof_night' || est ? .45 : lake ? .35 : .7})`); ctx.fillStyle = v; ctx.fillRect(0,0,cols*T,rows*T);
}
