...
function drawExplore(t){
  ...
  if (a.decor && (a.decor.startsWith('plain') || a.decor.startsWith('hills'))) {
    ...
    a.map.forEach((row, y) => [...row].forEach((ch, x) => {
      if (ch === 'L') { const fl = .85 + Math.sin(t/230 + x*1.7)*.08 + Math.sin(t/71 + y)*.04; glow(ctx, x*T + T/2, y*T + T*.2, T*3.4, '#6aa8ff', .42*fl); glow(ctx, x*T + T/2, y*T + T*.2, T*.9, '#bfe0ff', .5*fl); ctx.fillStyle = `rgba(160,210,255,${.7 + fl*.2})`; ctx.fillRect(x*T + T*.44, y*T + T*.15, T*.12, T*.12); }
  ...
  glow(ctx, d.x*T + T/2, d.y*T + T/2, T*2.6, '#e8b060', a.decor === 'pale' || a.decor === 'plain' || a.decor === 'hills' || a.decor === 'city_dusk' ? .06 : .14);
  // fog banks
  fog.forEach(f => { f.x += f.v; if (f.x > 1.3) f.x = -.3; ell(ctx, f.x*cols*T, f.y*rows*T, f.w*cols*T, T*.9, a.decor === 'plain' || a.decor === 'hills' ? `rgba(120,130,90,${f.a*.6})` : `rgba(70,62,58,${f.a})`); });
  // drifting ash
  if (!(a.decor && (a.decor.startsWith('plain') || a.decor.startsWith('hills')))) ash.forEach(s => { s.y += s.v; if (s.y > 1) s.y = 0; const x = (s.x + Math.sin(t/1600 + s.w)*.01) * cols*T; ctx.fillStyle = 'rgba(200,190,175,.4)'; ctx.fillRect(x, s.y*rows*T, s.s, s.s); });
  // darkness at the edges (deeper at night)
  const v = ctx.createRadialGradient(cols*T/2, rows*T/2, T*4, cols*T/2, rows*T/2, T*10); v.addColorStop(0, 'rgba(0,0,0,0)'); v.addColorStop(1, `rgba(0,0,0,${a.decor === 'pale' ? .6 : a.decor === 'plain' || a.decor === 'hills' ? .35 : a.decor === 'plain_dusk' || a.decor === 'hills_dusk' || a.decor === 'city_dusk' ? .5 : a.decor === 'city_night' || a.decor === 'roof_night' ? .45 : .7})`); ctx.fillStyle = v; ctx.fillRect(0,0,cols*T,rows*T);
}
