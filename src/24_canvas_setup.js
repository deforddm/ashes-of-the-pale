...
function fitCanvas(cols, rows){
  ...
  if (view === 'explore') { const a = AREA(); prerender(a.map, cols, rows, a.decor === 'camp_night' || a.decor === 'plain_night' || a.decor === 'city_night' || a.decor === 'cellar' || a.decor === 'roof_night', false, a.decor); } else if (view === 'battle' && B) prerender(B.def.map, 8, 10, B.def.dark, !B.def.open && !B.def.style, B.def.style || (B.def.open ? 'plain' : '')); 
}
