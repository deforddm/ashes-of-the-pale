...
function cardSequence(done, opt = {}){
  ...
  el.innerHTML = `<canvas id="ccv"></canvas><div class="cf" id="cfText"><div class="cn">${esc(c.name)}</div><div class="cfx">${esc(c.fx || '')}</div><div class="ct">${esc(c.txt || '')}</div><button class="btn primary" id="cfGo">${self ? 'Put the card away' : S.chapter ? "Put the cards away" : "Descend"}</button></div>`;
